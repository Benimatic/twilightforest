	...
	public void aiStep() {
		...
		if (this.level.isClientSide) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (this.level.getBiome(this.blockPosition()).value().shouldSnowGolemBurn(this.blockPosition()) || this.isOnFire()) {
		...
		if (this.level.getBiome(this.blockPosition()).value().shouldSnowGolemBurn(this.blockPosition()) && this.tickCount % 20 == 0) {
			//BURN!!!
