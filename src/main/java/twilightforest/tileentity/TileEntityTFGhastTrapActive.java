...
public class TileEntityTFGhastTrapActive extends TileEntity implements ITickableTileEntity {
	...
	@Override
	public void tick() {
		if (getBlockState().get(BlockTFGhastTrap.ACTIVE)) {
			tickActive();
		} else {
			tickInactive();
		}
	...
	public boolean receiveClientEvent(int event, int payload) {
		if (event == BlockTFGhastTrap.ACTIVATE_EVENT) {
			counter = 0;
			dyingGhasts.clear();
			return true;
		}
		if (event == BlockTFGhastTrap.DEACTIVATE_EVENT) {
			counter = 0;
			return true;
		}
		return false;
	...
	private void tickActive() {
		...
		if (world.isRemote) {
			...
			if (counter >= 120) {
				world.setBlockState(getPos(), getBlockState().with(BlockTFGhastTrap.ACTIVE, false));
				world.addBlockEvent(getPos(), getBlockState().getBlock(), BlockTFGhastTrap.DEACTIVATE_EVENT, 0);
			}
		}
	}
}
