...
public class FinalCastleMainComponent extends TFStructureComponentOld {
	...
	private void buildTowerMaze(List<StructurePiece> list, Random rand, int x, int y, int z, int howFar, Direction direction, BlockState type, BlockPos dest) {
		...
		while (!complete && iterations < 15) {
			...
			FinalCastleBridgeComponent bridge = new FinalCastleBridgeComponent(getFeatureType(), this.getComponentType() + 1, bc.getX(), bc.getY(), bc.getZ(), howFar - 7, direction);
	...
	private boolean isMazeComplete(List<StructurePiece> list, BlockState type) {
		...
		for (StructurePiece structurecomponent : list) {
			...
			if (type == TFBlocks.castle_rune_brick_pink.get().getDefaultState() && structurecomponent instanceof FinalCastleEntranceTowerComponent) {
				return true;
			}
			if (type == TFBlocks.castle_rune_brick_blue.get().getDefaultState() && structurecomponent instanceof FinalCastleBellTower21Component) {
				return true;
			}
		}
		return false;
