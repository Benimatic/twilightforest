...
public final class ConfiguredFeatures {
    ...
    private static final ConfiguredDecorator<?> DEFAULT_TREE_PLACEMENT = TwilightFeatures.CONFIGURED_PLACEMENT_NOTFSTRUCTURE_WITHHEIGHTMAP.decorated(Features.Decorators.HEIGHTMAP_WITH_TREE_THRESHOLD_SQUARED);/*.withPlacement(Placement.COUNT_EXTRA.configure(new AtSurfaceWithExtraConfig(8, 0.4F, 4)));*/ // FIXME: Execution order = Outer -> Inner (Last -> First); COUNT_EXTRA was first meaning it did nothing, so now properly placing it last causes mayhem. Reenable this and fix all the random values to match 1.12 again
    ...
    //FIXME generate via surface builder instead
    //public static final ConfiguredFeature<?, ?> THORNS = TwilightFeatures.registerWorldFeature(TwilightForestMod.prefix("thorns"), TFBiomeFeatures.THORNS.get().configured(FeatureConfiguration.NONE).decorated(TwilightFeatures.CONFIGURED_PLACEMENT_NOTFSTRUCTURE).rarity(95).decorated(Features.Decorators.HEIGHTMAP_WORLD_SURFACE).count(50).countRandom(85));
    public static final ConfiguredFeature<?, ?> TORCH_BERRIES = TwilightFeatures.registerWorldFeature(TwilightForestMod.prefix("torch_berries"), TFBiomeFeatures.TORCH_BERRIES.get().configured(FeatureConfiguration.NONE).decorated(Features.Decorators.HEIGHTMAP_WORLD_SURFACE).squared().count(4));
    public static final ConfiguredFeature<?, ?> TROLL_ROOTS = TwilightFeatures.registerWorldFeature(TwilightForestMod.prefix("troll_roots"), TFBiomeFeatures.TROLL_ROOTS.get().configured(FeatureConfiguration.NONE).decorated(Features.Decorators.HEIGHTMAP_WORLD_SURFACE).squared().count(4).countRandom(8));
