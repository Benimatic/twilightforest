...
public final class BiomeMaker extends BiomeHelper {
	...
	private static void swampBiomes(ImmutableMap.Builder<RegistryKey<Biome>, Biome> biomes) {
		biomes.put(BiomeKeys.SWAMP,
				biomeWithDefaults(defaultAmbientBuilder().withGrassColor(0x5C694E).withFoliageColor(0x496137).setWaterColor(0xE0FFAE), defaultMobSpawning(), swampGen(defaultGenSettingBuilder()).withStructure(TFStructures.CONFIGURED_LABYRINTH))
		...
		biomes.put(BiomeKeys.FIRE_SWAMP,
				biomeWithDefaults(whiteAshParticles(defaultAmbientBuilder().withGrassColor(0x572E23).withFoliageColor(0x64260F).setWaterColor(0x6C2C2C)), defaultMobSpawning(), fireSwampGen(defaultGenSettingBuilder()).withStructure(TFStructures.CONFIGURED_HYDRA_LAIR))
						.category(Biome.Category.SWAMP)
						.temperature(1)
						.downfall(0.4F)
						.depth(0.1F)
	...
	private static void darkForestBiomes(ImmutableMap.Builder<RegistryKey<Biome>, Biome> biomes) {
		biomes.put(BiomeKeys.DARK_FOREST,
				biomeWithDefaults(defaultAmbientBuilder().withGrassColor(0x4B6754).withFoliageColor(0x3B5E3F), defaultMobSpawning(), darkForestGen().withStructure(TFStructures.CONFIGURED_KNIGHT_STRONGHOLD))
						.temperature(0.7F)
						.downfall(0.8F)
						.depth(0.125F)
						.scale(0.05F)
		...
		biomes.put(BiomeKeys.DARK_FOREST_CENTER, // FIXME: colors
				biomeWithDefaults(defaultAmbientBuilder().withGrassColor(0x667540).withFoliageColor(0xF9821E), defaultMobSpawning(), darkForestCenterGen().withStructure(TFStructures.CONFIGURED_DARK_TOWER))
						.depth(0.125F)
						.scale(0.05F)
						.build()
		);
	...
	private static void snowRegionBiomes(ImmutableMap.Builder<RegistryKey<Biome>, Biome> biomes) {
		biomes.put(BiomeKeys.SNOWY_FOREST,
				biomeWithDefaults(defaultAmbientBuilder(), defaultMobSpawning(), snowyForestGen(defaultGenSettingBuilder()).withStructure(TFStructures.CONFIGURED_YETI_CAVE))
		...
		biomes.put(BiomeKeys.GLACIER,
				biomeWithDefaults(defaultAmbientBuilder(), defaultMobSpawning(), glacierGen().withStructure(TFStructures.CONFIGURED_AURORA_PALACE))
	...
	private static void highlandsBiomes(ImmutableMap.Builder<RegistryKey<Biome>, Biome> biomes) {
		...
		biomes.put(BiomeKeys.FINAL_PLATEAU,
				biomeWithDefaults(defaultAmbientBuilder(), new MobSpawnInfo.Builder(), defaultGenSettingBuilder().withStructure(TFStructures.CONFIGURED_FINAL_CASTLE))
						.category(Biome.Category.MESA)
						.temperature(0.3F)
						.downfall(0.2F)
						.depth(10.5F)
