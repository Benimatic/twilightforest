...
public class TFBiomes {
	...
	public static void bootstrap(BootstapContext<Biome> context) {
		...
		context.register(ENCHANTED_FOREST, biomeWithDefaults(fireflyParticles(defaultAmbientBuilder()).foliageColorOverride(0x00FFFF).grassColorOverride(0x00FFFF).grassColorModifier(BiomeGrassColors.ENCHANTED_FOREST), defaultMobSpawning(), enchantedForestGen(featureGetter, carverGetter)).precipitation(Biome.Precipitation.NONE).build());
		...
		context.register(SNOWY_FOREST, biomeWithDefaults(defaultAmbientBuilder().skyColor(0x808080).fogColor(0xFFFFFF).foliageColorOverride(0xFFFFFF).grassColorOverride(0xFFFFFF), snowForestSpawning(), snowyForestGen(featureGetter, carverGetter)).precipitation(Biome.Precipitation.SNOW).temperature(0.09F).downfall(0.9F).build());
		context.register(GLACIER, biomeWithDefaults(defaultAmbientBuilder().skyColor(0x130D28).fogColor(0x361F88), penguinSpawning(), glacierGen(featureGetter, carverGetter)).temperature(0.8F).downfall(0.1F).precipitation(Biome.Precipitation.SNOW).build());
