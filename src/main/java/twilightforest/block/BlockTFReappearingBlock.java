...
public class BlockTFReappearingBlock extends Block {

	public static final BooleanProperty ACTIVE = BooleanProperty.create("active");

	public BlockTFReappearingBlock() {
	...
	public float getBlockHardness(BlockState state, IBlockReader world, BlockPos pos) {
		return state.get(ACTIVE) ? -1 : super.getBlockHardness(state, world, pos);
	}
	...
	public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
		...
		if (state.get(ACTIVE)) {
			world.setBlockState(pos, TFBlocks.reappearing_translucent.get().getDefaultState().with(BlockTFReappearTranslucent.ACTIVE, false));
	...
	public static void changeToActiveVanishBlock(World world, BlockPos pos, boolean variant) {
		changeToActiveVanishBlock(world, pos, TFBlocks.reappearing_block.get().getDefaultState().with(ACTIVE, variant));
	}

	/**
	 * Change this block into an active vanishing block
	 */
	public static void changeToActiveVanishBlock(World world, BlockPos pos, BlockState state) {
...
//		if (state.getBlock() == TFBlocks.reappearing_block.get() && state.get(ACTIVE)) {
