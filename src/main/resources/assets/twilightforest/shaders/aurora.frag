...
float noiseTwoPass(vec4 v) {
    return snoise(v) + snoise(v/2.0);
}
...
void main() {
    ...
    blurred = texture2D(zero, texCoord0) * weight[0];

    for (int i=1; i<5; i++) {
    	blurred += texture2D(zero, texCoord0 + offset[i]/texSize.y) * weight[i];
    	blurred += texture2D(zero, texCoord0 + offset[i]/texSize.y) * weight[i];
    }

    float posZ = sqrt(max(0.0, -position.z))/10.0;
    float val = posZ + getPerceptualBrightness(color.rgb) + time/100.0;
                                                                                          // 0.67 - 0.37 = 0.3
